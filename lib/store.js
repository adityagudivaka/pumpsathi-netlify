...
function bs() {
  // Default (eventual) consistency — 'strong' needs an uncachedEdgeURL that
  // the Lambda-compat context doesn't provide. Eventual is fine here.
  const opts = { name: 'pumpsathi' };
...
async function setUserAuth(id, hash, role) {
  const users = await listUsersRaw(); const u = users.find((x) => x.id === id);
  if (!u) return false;
  u.password_hash = hash; if (role) u.role = role;
  await setJSON('users', users); return true;
}
...
// Per-firm audit trail (capped)
async function audit(firmId, actor, action, area, entityId) {
  const key = `f/${firmId}/audit`;
  const log = await getJSON(key, []);
  log.unshift({ ts: new Date().toISOString(), actor: actor || '', action, area: area || '', id: entityId || '' });
  await setJSON(key, log.slice(0, 1000));
}
async function auditList(firmId) { return getJSON(`f/${firmId}/audit`, []); }

module.exports = {
  listAll, insert, update, remove, clearCollection, kvGet, kvSet,
  listFirms, getFirm, createFirm, userByMobile, usersOfFirm, userCount, createUser, deleteUser, setUserAuth,
  waLog, waRecent, stampDone, stampMark, audit, auditList,
};
